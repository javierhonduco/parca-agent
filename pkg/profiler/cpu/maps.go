...
func (m *bpfMaps) setUnwindTable(pid int, ut unwind.UnwindTable) error {
	...
	for i := 0; i < len(ut); i += maxUnwindTableSize {
		...
		// Write `.low_pc`.
		if err := binary.Write(buf, m.byteOrder, uint64(chunk[0].Loc)); err != nil {
			return fmt.Errorf("write the number of rows: %w", err)
		}
		// Write `.high_pc`.
		if err := binary.Write(buf, m.byteOrder, uint64(chunk[len(chunk)-1].Loc)); err != nil {
			return fmt.Errorf("write the number of rows: %w", err)
		}
	...
	mapShard := []*libbpfgo.BPFMap{m.unwindTable1, m.unwindTable2, m.unwindTable3}
	...
	for idx, buf := range shardBufs {
		...
		if err := mapShard[idx].Update(unsafe.Pointer(&pid), unsafe.Pointer(&buf.Bytes()[0])); err != nil {
