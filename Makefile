...
test_profiler: $(GO_SRC) $(LIBBPF_HEADERS) $(LIBBPF_OBJ) bpf
	sudo -E $(go_env) go test -v $(shell go list ./... | grep "pkg/profiler") $(SANITIZER)

.PHONY: test
ifndef DOCKER
test: $(GO_SRC) $(LIBBPF_HEADERS) $(LIBBPF_OBJ) bpf test_profiler
