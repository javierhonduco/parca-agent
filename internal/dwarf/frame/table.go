...
type DWRule struct {
	Rule Rule
	// TODO(javierhonduco): this type is too large for what we need
	Offset int64
	// TODO(javierhonduco): same as above + use the 0 value as a especial value and encode
	// registers starting from 1 to ensure that the values we get are not bogus
	Reg        uint64
	Expression []byte
}
...
type InstructionContext struct {
	...
	address uint64
	CFA     DWRule
	// TODO: do we need all the DWRule mappings below??
